...
class LocalOpenAICompatibleJudgeProvider(JudgeProvider):

    def __init__(self, model: str, base_url: str, api_key_env: str | None = None, timeout_seconds: int = 30, max_retries: int = 2):
        self.model = model
        ...
        try:
            ...
            if not tc:
                return JudgeResult(
                    verdict="needs_human_review",
                    confidence=0,
                    exploit_explanation="Local OpenAI tool choice was ignored or did not return a tool call",
                    suggested_fix=""
                )
            ...
            if not verdict or confidence is None or exploit_explanation is None or suggested_fix is None:
                return JudgeResult(
                    verdict="needs_human_review",
                    confidence=0,
                    exploit_explanation="Local OpenAI response was missing required fields",
                    suggested_fix=""
                )
            ...
            return JudgeResult(
                verdict="needs_human_review",
                confidence=0,
                exploit_explanation=f"Local OpenAI API call failed: {e}",
                suggested_fix=""
            )
